...
#define TSMINER_TIMEOUT_MS 860000
#endif
...
#ifdef USE_TSMINER
static struct usb_epinfo tsm_epinfos[] = {  
    { LIBUSB_TRANSFER_TYPE_BULK, 64, EPI(1), 0, 0 },  
    { LIBUSB_TRANSFER_TYPE_BULK, 64, EPO(2), 0, 0 }  
};  
  
static struct usb_intinfo tsm_ints[] = {  
    USB_EPS(0, tsm_epinfos)  
};
#endif
...
static struct usb_find_devices find_dev[] = {
	...
	{  
		...
		.name = "TSM",  
		.ident = IDENT_TSM,  
		.idVendor = 0x0403,        // Vendor ID FTDI  
		.idProduct = 0x6001,       // Product ID FT232  
		.iManufacturer = NULL,  
		.iProduct = NULL,  
		.config = 1,  
		.timeout = TSMINER_TIMEOUT_MS,
		.latency = LATENCY_STD,  
		INTINFO(tsm_ints)  
	},
#endif
...
static bool usb_check_device(struct device_drv *drv, struct libusb_device *dev, struct usb_find_devices *look)
{
	...
	//parche
/*	if (drv->drv_id == DRIVER_tsminer && desc.idVendor == 0x0403 && desc.idProduct == 0x6001) {  
		applog(LOG_DEBUG, "%s forcing match for TSM %04x:%04x", drv->name, desc.idVendor, desc.idProduct);  
		return true;  
	}  */
	//fin parch
	...
	for (i = 0; i < total_devices; i++) {
		...
		switch (cgpu->drv->drv_id) {
			...
			case DRIVER_hashfast:
				...
				count++;
				break;
			case DRIVER_tsminer:  // Agregar esta línea  
        		DEVWLOCK(cgpu, pstate);  
        		release_cgpu(cgpu);  
        		DEVWUNLOCK(cgpu, pstate);  
        		count++;  
